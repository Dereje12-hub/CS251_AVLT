...
//Funciton used to delate/free all nodes
//
...
void _inorderTraversal(ostream& output, NODE* cur) const
	...
	else
    {
          if ((cur->isThreaded) && (cur->Right != NULL)){
			  ...
			  //call inorder traversal to the left and print the output respectively
			   //
			   ...
			   {
				  //call inorder traversal to the left and print the output  and 
  ...
  {         
		...
		if (L->isThreaded )     //condition which check if L is treaded and set L->Left NULL
			 N->Left = NULL;
...
public:
  ...
  void insert(KeyT key, ValueT value)
  {
   ...
   // 
    ...
    //if we get here, key is not in tree, so allocate
    // a new node to insert:
    // initialized as treades and height is 0
    // 
    NODE* newNode;
    newNode = new NODE();
    newNode->Key = key;
 ...
 void begin()
  {
	...
	//
    // condition if the Root is NOT NULL go far left 
    //
     if (Begin != NULL){
