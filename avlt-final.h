...
void _inorderTraversal(ostream& output, NODE* cur) const
  ...
  {         
	   ...
	   int NR = (N->Right == nullptr|| N->Right->isThreaded) ? -1:N->Right->Height;

		N->Height = 1 + std::max(NL, NR);


		int LL = (L->Left ==nullptr) ? -1 : L->Left->Height; 
        int LR = (L->Right==nullptr || L->Right->isThreaded) ? -1: L->Right->Height;
  ...
  void _LeftRotate(NODE* Parent, NODE* N)
  {
    ...
    if(Parent == NULL)
     ...
     //update height
		...
		int RR = (R->Right == nullptr || R->Right->isThreaded) ? -1 : R->Right->Height;
 ...
 void _RotateToFix(NODE* Parent, NODE* N)
	{
		...
		//condtion to check and to rotate if the height differce is > 1
		//
		if (std::abs(hL - hR) > 1){	
		    if (hL > hR )
		     {
        ...
        int hLR = (N->Left->Right == nullptr||N->Left->isThreaded)?-1:N->Left->Right->Height;
			...
			if (hLL > hLR)              // condition that check for far Left 	  
				_RightRotate(Parent, N);    //call to right rotate  
			
			else                    // condition that check for middle Left  
			    {		
...
////////////////////////////////////////////////////
 ...
 //
  ...
  vector<KeyT> range_search(KeyT lower, KeyT upper)
  {
	...
	while(cur != NULL){
		if ( cur->Key <= lower ){
		  // now the pointer which is pointing 
		  // on the lower is found as ptr
		  //  
          //cur = ptr; 
	//	  cout<< "I am in begin while 1"<< endl;
		  break;
